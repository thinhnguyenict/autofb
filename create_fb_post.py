...
def create_post(page_id, message, access_token, post_url, photo_path):
    post_url = f'{post_url}/{page_id}/photos'
    payload = {
    ...
    try:
        ...
        if 'post_id' in response:
            logging.info(f"Success - PostID: {response['post_id']}")
            ...
            logging.error("Failed to create new post")
...
def main():
    for page_id, access_token in page_access_tokens.items():
        ...
        create_post(page_id, message, access_token, api_url, photo_path)
